...
def write_config_file(name):
    ...
    # print("Invasion_GUI.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)


# callback from write_config_button
# def write_config_file_cb(b):
#     path_to_share = os.path.join('~', '.local','share','Invasion_GUI')
...
def get_config_files():
    ...
    files = [file for file in glob.glob("%s/PhysiCell_settings_*.xml" % dirname)]
    # dict.update() will append any new (key,value) pairs
    for file in files:
        cf.update({os.path.basename(file)[19:-4] : file})
    # cf.update(dict(zip(list(map(os.path.basename, files)), files)))

    # Find the dir path (full_path) to the cached dirs
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/Invasion_GUI")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "Invasion_GUI")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB Invasion_GUI' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 Invasion_GUI-r7 config.xml")   # "-r7" suffix??
        else:
...
tool_title = widgets.Label(r'\(\textbf{Invasion_GUI}\)')
if nanoHUB_flag or hublib_flag:
